...
- (void)setBeautyTitle:(NSString *)title {
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([title isEqualToString:@"race"]) {
            NSString *beautyTitle = [@"高级美颜美型由?提供" localString];
            beautyTitle = [beautyTitle stringByReplacingOccurrencesOfString:@"?" withString:title];
        ...
        }else if ([title isEqualToString:@"faceunity"]){
            NSString *beautyTitle = [@"高级美颜美型由?提供  如何获取" localString];
            beautyTitle = [beautyTitle stringByReplacingOccurrencesOfString:@"?" withString:title];
            NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:beautyTitle attributes:@{NSForegroundColorAttributeName:[UIColor whiteColor],NSFontAttributeName:[UIFont systemFontOfSize:12]}];
